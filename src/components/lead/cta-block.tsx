...
export function CtaBlock({
  ...
  return (
    <aside className="rounded-xl bg-gradient-to-br from-indigo-600 to-indigo-800 p-6 text-white print:hidden">
      <p className="text-xs font-semibold uppercase tracking-wide text-indigo-200">
        EBOS {moduleName} Module
      </p>
      <h2 className="mt-1 text-xl font-bold">{headline}</h2>
      <p className="mt-2 text-sm text-indigo-100">{body}</p>
      <a
        href={href}
        target="_blank"
        rel="noopener"
        ...
        className="mt-4 inline-block rounded-md bg-white px-5 py-2.5 text-sm font-semibold text-indigo-700 hover:bg-indigo-50"
      >
