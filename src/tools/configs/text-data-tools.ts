...
export const tools: ToolConfig[] = [
  {
    ...
    autoCompute: true,
    about: [
      "This word counter shows live counts of words, characters (with and without spaces), sentences, paragraphs, reading time and speaking time as you type or paste. Whether you're writing a blog post with a target length, a meta description with a character limit, a college essay with a word requirement, or a speech that has to fit a time slot, every metric updates instantly as you edit. Nothing is uploaded or stored; the counting happens entirely in your browser, so it's safe for confidential drafts and works even on a flaky connection.",
  ...
  },
  {
    ...
    autoCompute: true,
    about: [
      "This counter gives you live character counts as you type or paste — with a separate count that excludes spaces for platforms and forms that measure that way. Half of modern writing happens against a character limit: 160 characters for an SMS, 280 for a post on X, 150–160 for a meta description, 30 for a Google Ads headline, 125 before an Instagram caption truncates.",
  ...
  },
  {
    ...
    submitLabel: "Convert",
    about: [
      "This converter fixes text stuck in the wrong case in one click: paste the text, pick the target case, convert, and copy the result. Retyping text because it arrived in the wrong case is a small, constant tax on anyone who writes or codes. It handles the everyday writing cases — UPPERCASE for headings and legal emphasis, lowercase to calm down text typed with caps lock on, Title Case for headlines, and Sentence case to restore normal prose capitalisation after each full stop.",
  ...
  },
  {
    ...
    submitLabel: "Format",
    about: [
      "This formatter turns unreadable, single-line JSON into cleanly indented, human-readable structure in one click — or does the reverse, minifying a formatted document back to its most compact form for production payloads. JSON is how systems talk to each other — API responses, configuration files, webhooks, exports — but machines emit it as a single unreadable line.",
  ...
  },
  {
    ...
    submitLabel: "Convert",
    about: [
      "This converter turns CSV into a JSON array: paste your CSV, choose the delimiter, and get a JSON array ready to use. CSV is the language of spreadsheets while JSON is the language of APIs and applications, so moving data from one world to the other is a constant chore — importing a customer list into a web app, seeding a database from an Excel export, feeding spreadsheet data to a script.",
  ...
  },
  {
    ...
    submitLabel: "Convert",
    about: [
      "This tool turns a JSON array into CSV: paste an array of objects and it produces a CSV with a header row assembled from the objects' keys, one line per record, ready to save and open in any spreadsheet application. APIs speak JSON, but analysis happens in spreadsheets, so when you need to eyeball an API response, share data with a colleague who lives in Excel, or import records into accounting software, converting a JSON array to CSV is the bridge.",
  ...
  },
  {
    ...
    submitLabel: "Compare",
    about: [
      "This tool compares two texts line by line and shows exactly what changed: paste the original in the first box and the new version in the second, and you get a report with unchanged lines plain, removed lines marked with a minus, and added lines marked with a plus — the same convention developers have trusted in code diffs for decades. That answers \"what exactly did they change?\" — the question behind every contract revision, edited article, updated policy and reworked email, where reading two versions side by side and spotting differences by eye is slow and unreliable.",
  ...
  },
  {
    ...
    about: [
      "A randomly generated password has no pattern to exploit: cracking it means trying the full space of possibilities, and that space grows astronomically with length and character variety. That matters because the passwords people invent instead follow patterns — a name, a year, an exclamation mark — and attackers' tools know every one of them. A 16-character password drawn from all four character sets has more combinations than there are atoms in a human body; at any realistic guessing rate, it is simply out of reach.",
  ...
  },
  {
    ...
    submitLabel: "Replace",
    about: [
      "This tool gives you full find-and-replace in the browser: paste any text, say what to find and what to put in its place, and take the result away — with three switches that separate a clean edit from a mangled one. Every editor has find-and-replace, but it's often locked inside a document, limited to one file format, or missing those options that make bulk edits safe.",
