...
export function RotatePdf() {
  ...
  const run = async () => {
    if (!file || pageCount === null) return;
    let selected: number[] | null = Array.from({ length: pageCount }, (_, i) => i);
    if (scope === "range") {
      selected = parsePageRanges(ranges, pageCount);
      if (!selected) {
        setError('Enter pages like "2, 4-6".');
        return;
      }
    ...
    <div className="space-y-4">
      ...
      {file && (
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="rot-angle" className={labelCls}>Rotate by</label>
            <select id="rot-angle" className={inputCls} value={angle} onChange={(e) => setAngle(e.target.value)}>
          ...
          <div>
            <label htmlFor="rot-scope" className={labelCls}>Apply to</label>
            <select id="rot-scope" className={inputCls} value={scope} onChange={(e) => setScope(e.target.value)}>
          ...
          {scope === "range" && (
            <div className="sm:col-span-2">
              <label htmlFor="rot-ranges" className={labelCls}>Pages (e.g. &quot;2, 4-6&quot;)</label>
              <input id="rot-ranges" type="text" className={inputCls} value={ranges}
                onChange={(e) => setRanges(e.target.value)} placeholder="2, 4-6" />
            </div>
          )}
        </div>
      )}
